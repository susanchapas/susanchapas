...
**Self-description.** _"The Strategic Architect — bridging design, marketing strategy, and technical
implementation."_

**Positioning line.** _"I'm a designer who refuses to stop at the mockup."_ Everything in this system serves
...
board. Self-described as _"neo-modern Swiss style with strong grids and negative space"_ and _"cinematic motion
design."_
...
12. **There is no logo.** Set `SC` or `Susan Chapas` in Instrument Sans Bold, mint on midnight. Never draw, reconstruct or approximate a mark.
...
:root {
  ...
  --sky-a10: rgba(187, 205, 243, 0.1);
  --sky-a15: rgba(187, 205, 243, 0.15);
  --sky-a20: rgba(187, 205, 243, 0.2);
  --sky-a30: rgba(187, 205, 243, 0.3);
  --mint-a10: rgba(111, 205, 157, 0.1);
  --mint-a20: rgba(111, 205, 157, 0.2);
  --mint-a30: rgba(111, 205, 157, 0.3);
  --mint-a50: rgba(111, 205, 157, 0.5);
  --paper-a05: rgba(244, 244, 245, 0.05);
  --paper-a40: rgba(244, 244, 245, 0.4);
  --paper-a70: rgba(244, 244, 245, 0.7);
  --paper-a80: rgba(244, 244, 245, 0.8);
  ...
  --surface-glass: rgba(16, 47, 93, 0.7);
  ...
  --text-link: var(--mint);
  --text-link-hover: rgba(111, 205, 157, 0.8);
  ...
  --gradient-mesh:
    radial-gradient(ellipse at 20% 80%, rgba(187, 205, 243, 0.15) 0%, transparent 50%),
    radial-gradient(ellipse at 80% 20%, rgba(111, 205, 157, 0.08) 0%, transparent 50%),
    radial-gradient(ellipse at 40% 40%, rgba(224, 159, 125, 0.1) 0%, transparent 50%),
    var(--midnight);
  --gradient-protect: linear-gradient(
    to top,
    var(--midnight) 0%,
    rgba(16, 47, 93, 0.55) 45%,
    transparent 100%
  );
}
...
:root {
  ...
  --shadow-tile:
    0 20px 25px -5px rgba(0, 0, 0, 0.3), 0 8px 10px -6px rgba(0, 0, 0, 0.3); /* shadow-xl on studio tiles */
  ...
  --ring-tile:
    inset 0 1px 0 rgba(187, 205, 243, 0.08), inset 0 0 40px rgba(0, 0, 0, 0.35); /* @kind shadow */
...
@keyframes sc-float-slow-reverse {
  0%,
  100% {
    transform: translateY(0) translateX(0);
  }
...
a {
  color: var(--text-link);
  text-decoration-color: var(--mint-a30);
...
@theme inline {
  --color-page: var(--midnight);
  --color-panel: var(--midnight-deep);
...
Then: `bg-page text-paper`, `bg-accent text-page`, `border-sky/10`, `rounded-full`,
...
```json
{
  ...
  "alpha": {
    "sky": { "a05": 0.05, "a10": 0.1, "a15": 0.15, "a20": 0.2, "a30": 0.3 },
    "mint": { "a10": 0.1, "a20": 0.2, "a30": 0.3, "a50": 0.5 },
    "paper": { "a05": 0.05, "a40": 0.4, "a70": 0.7, "a80": 0.8 },
    "ink": { "a25": 0.25 }
  },
  "font": {
  ...
  "motion": {
    ...
    "durationBase": 300,
    "durationSlow": 500,
    "durationEntrance": 700,
    "durationMarquee": 60000,
    "hoverLift": 1.02,
    "pressScale": 0.98,
    "imageZoom": 1.05
  },
...
| `--midnight-deep` | `#091a35` | —                    | Only for things floating _above_ the page: tooltips, drawers, detail panels.     |
...
<section style="position:relative;overflow:hidden;padding:8rem 0">
  <div class="sc-gradient-mesh" style="position:absolute;inset:0">
    <div
      style="position:absolute;top:25%;left:20%;height:384px;width:384px;border-radius:50%;
                background:var(--mint-a10);filter:blur(48px);
                animation:sc-float-slow 15s ease-in-out infinite"
    ></div>
    <div
      style="position:absolute;right:18%;bottom:18%;height:320px;width:320px;border-radius:50%;
                background:var(--sky-a15);filter:blur(48px);
                animation:sc-float-slow-reverse 18s ease-in-out infinite"
    ></div>
  </div>
...
> **Substitution note.** Upstream uses _Space Grotesk_ (display) and _DM Sans_ (body); its README also mentions
> _Syne_. Space Grotesk was deliberately replaced. Instrument Sans keeps the geometric confidence with smoother
...
| Tile kicker      | body    | `0.65rem`                   | 600       | 1.5     | `0.2em`, uppercase | `rgba(16,47,93,0.50)`                         |
...
<h2
  style="font-family:var(--font-display);font-size:var(--text-4xl);font-weight:700;
           line-height:1.1;letter-spacing:-0.02em;margin-bottom:1.5rem"
>
  Selected <span class="sc-text-gradient">Case Studies</span>
</h2>
<p
  style="max-width:var(--measure);font-size:var(--text-lg);color:var(--text-muted);margin:0"
>
...
function justify(items, containerWidth) {
  ...
  const flush = (isLast) => {
    ...
    rows.push(
      row.map((a) => ({
        art: a,
        w: Math.round(h * (a.width / a.height)),
        h: Math.round(h),
      }))
    );
    row = [];
...
**The signature grid behaviour:** hovering one tile dims _every other_ tile. Track hover in the grid and pass
...
**Panel** — `--surface-page` (plain midnight, _not_ the deep panel, because the media bed behind it is 30%
...
<Modal
  open={!!active}
  stacked={active.width / active.height >= 1.3}
  onClose={close}
  onPrev={prev}
  onNext={next}
  media={
...
2. **Sticky control bar** _(Gallery only)_ — `--surface-page`, `--border-subtle` bottom, `padding: 1.5rem 0`,
...
4. **Closing CTA** _(Projects only)_ — `--surface-card` section, centred `SectionHeading` size `md`
...
**First person, plainspoken, confident without swagger.** Susan writes as _I_, and addresses the reader as _you_
...
| ------------ | -------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| **Measured** | Case studies, role descriptions, contact page, capability copy | _"Leading website redesign, copywriting, translations, and UX improvements informed by user flow analysis."_     |
| **Relaxed**  | The About board, personal detail                               | _"It counts as water, right?"_ · _"No picky eaters allowed"_ · _"Chasing Korok seeds"_ · _"Did the scary thing"_ |
...
- _"I'm a designer who refuses to stop at the **mockup**."_
- _"Designer, developer, artist — **and a few more hats**."_
- _"Paint, Pixels **& Motion**."_
- _"Selected **Case Studies**"_
- _"Let's Create **Something Great**"_
...
**Kickers** name the section in one or two words: _Portfolio · Currently · Creative Side · About Me ·
Selected Work · Get in Touch_.

**Buttons and links are verb-first and specific.** Approved: _View My Work · Get in Touch · View Case Study ·
Start a Conversation · Send Message · Send Another Message · See the gallery · Read the feature · Watch the
full explainer · View All Projects · Previous role · Hide previous role · Skip to main content_.
Banned: _Learn more · Submit · Click here · Read more · Get started · Explore_.
...
`Category • Year`. Curly apostrophes. Oxford comma. Date ranges with a spaced en dash: _2025 — Present_.
...
**Banned words and habits** — synergy · leverage (as a verb) · passionate about · seamless · cutting-edge · drive · gap · showing up · shape · drive · chase · actual · real · shift
revolutionary · game-changing · "in today's fast-paced world" · third-person self-description · hedging ("I try
to…") · exclamation marks beyond genuine warmth (_"Drop me a message!"_ is the ceiling) · emoji.

### 13.5 Approved string library

Reuse these verbatim where they fit rather than paraphrasing.

**Positioning** — _"The Strategic Architect — bridging design, marketing strategy, and technical
implementation."_

**Hero** — _"UX Strategist & Marketing Professional"_ / _"I'm a designer who refuses to stop at the mockup."_ /
_"Susan Chapas — UX strategist, front-end developer, and award-winning artist. I figure out what people actually
need, design it to work for everyone, then build and ship it myself."_

**Selected Work lead** — _"A collection of projects where strategy meets execution. Each case study demonstrates
the intersection of design thinking, marketing expertise, and technical implementation."_

**Creative Side lead** — _"Exploring the intersection of humanity and technology through visual art."_

**About lead** — _"Everything that shapes how I work, laid out on the board below."_

**Contact lead** — _"I'm always open to discussing new opportunities, creative projects, or ways we can
collaborate. Drop me a message!"_

**Availability** — _"Available for opportunities"_ / _"Currently open to full-time roles, freelance projects, and
creative collaborations."_

**Projects CTA** — _"Have a project in mind?"_ / _"I'm always open to discussing new opportunities and creative
challenges. Let's create something meaningful together."_

**Success** — _"Message Sent!"_ / _"Thank you for reaching out. I'll get back to you as soon as possible."_

**Footer** — _"© {year} Susan Chapas. All rights reserved."_ / _"Jersey City"_
...
- **Link destinations.** Every external link gets `target="_blank" rel="noopener noreferrer"` **plus** a
  visually hidden _"(opens in new tab)"_. In-page anchors never open new tabs.
...
- **Attribution.** Collaborative work says so (_"Built as a team effort"_). Third-party contributions are named
  (_"shot by Spring Bank's videography partner and edited by me"_). Client work names the client. Awards state
  issuer and year.
...
| Text contrast       | Mint on midnight 6.8:1 · paper 12.4:1 · 70% paper 7.4:1. Never below 70% opacity for prose                                                              |
...
- [ ] No accent colour on a light surface without an ink variant
- [ ] Contrast-check any new colour pairing, both directions
...
pill radius if it's interactive, and document it — name, purpose, props, states, and _why_ it was needed.
...
- **The accent is substituted.** Upstream is Electric Lime `#ccff00` under the name `--accent-lime`. This
  system uses Mint `#6fcd9d`. Green now carries "success" semantics — if you build anything stateful, pair the
